...
### Удаляем балансировщик ( examples/delete.sh)
...
TARGET_GROUP_ID=$(curl -X GET  --silent -H "Authorization: Bearer $(yc iam create-token)"  \
 ...
 curl -X DELETE  -H "Authorization: Bearer $(yc iam create-token)"  \
  -H "Content-Type: application/json" \
  -k "https://load-balancer.api.cloud.yandex.net/load-balancer/v1/networkLoadBalancers/${LB_ID}"
...
echo "Deleting TG"
 ...
 curl -X DELETE  -H "Authorization: Bearer $(yc iam create-token)"  \
  -H "Content-Type: application/json" \
  -k "https://load-balancer.api.cloud.yandex.net/load-balancer/v1/targetGroups/${TARGET_GROUP_ID}"
```

### Удаляем инфраструктуру через terraform
